...
class PostForm(FlaskForm):
    content = TextAreaField("Username", validators=[DataRequired(), Length(min=15, max=1000)], render_kw={"placeholder": "Your Review"})
...
def dorm(dorm_name):
    ...
    return render_template("dorm.html", dorm=dorm, posts=posts,  dorm_avg_reviews= dorm_avg_reviews, title=title)
...
def create_post(dorm_name):
    ...
    form = PostForm()
    if form.validate_on_submit():
        new_post = Post(dorm=dorm_name, content=form.content.data, review=form.review.data, likes=0, date_posted=datetime.utcnow())
